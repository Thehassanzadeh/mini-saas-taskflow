...
async def get_authenticated_user(
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="decoded token failed")
...
async def get_authenticated_admin(
    ...
    if not user_obj.role_id == 2: # 2 is id for admin
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="you are not allowed to get this")
    return user_obj
